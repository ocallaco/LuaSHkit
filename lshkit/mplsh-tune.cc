...
FloatMatrix *getMatrix(int dim, int N, float *data_block){
    return new FloatMatrix(dim,N,data_block);
}

int tune (FloatMatrix *data, int T, int L, double R)
{
    int M = -1;
    double W = -1;
    unsigned N = data->getSize();
    unsigned K = 20;
    bool do_K = true;
